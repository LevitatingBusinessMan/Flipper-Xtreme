...
static void render_callback(Canvas* const canvas, void* ctx) {
    ...
    elements_multiline_text_aligned(canvas, 64, 2, AlignCenter, AlignTop, "IR Flashlight");
...
static void flash_toggle(PluginState* const plugin_state) {
    ...
    furi_hal_gpio_write(&gpio_infrared_tx, false);
    furi_hal_gpio_init(&gpio_infrared_tx, GpioModeOutputPushPull, GpioPullUp, GpioSpeedVeryHigh);
    ...
    if(plugin_state->is_on) {
        ...
        furi_hal_gpio_write(&gpio_infrared_tx, true);
        plugin_state->is_on = true;
    }
}

int32_t ir_flashlight_app() {
    ...
    if(!plugin_state->mutex) {
        FURI_LOG_E("ir_flashlight", "cannot create mutex\r\n");
